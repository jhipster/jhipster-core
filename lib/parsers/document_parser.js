...
function init(passedConfiguration, options = {}) {
  const { deferValidation } = options;
  const { skippedUserManagement } = passedConfiguration;
  configuration = passedConfiguration;
  jdlObject = new ValidatedJDLObject({ deferValidation, skippedUserManagement });
