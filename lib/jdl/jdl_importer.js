...
function importOnlyEntities(jdlObject, configuration) {
  ...
  const jsonEntities = entitiesPerApplicationMap.get(applicationName).reduce((previous, current) => {
...
function importApplicationsAndEntities(jdlObject, configuration) {
  ...
  if (jdlObject.getApplicationQuantity() === 1) {
    importState.exportedApplications.push(
      JHipsterApplicationExporter.exportApplication(jdlObject.getApplications()[0], configuration)
    );
  } else {
  ...
  entitiesPerApplicationMap.forEach((entities, applicationName) => {
    ...
    const exportedJSONEntities = exportJSONEntities(jsonEntities, {
      ...
      forSeveralApplications: jdlObject.getApplicationQuantity() !== 1
    });
