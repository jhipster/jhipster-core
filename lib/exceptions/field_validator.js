...
class FieldValidator extends Validator {
  ...
  validate(jdlField) {
    ...
    checkValidations(jdlField);
  }
}

module.exports = FieldValidator;

function checkValidations(jdlField) {
  ...
  jdlField.forEachValidation((validation, index) => {
    try {
      ...
      validationValidator.validate(validation);
    } catch (error) {
  ...
  throw new Error(formatValidationErrors(jdlField, errorsPerValidation));
}

function formatValidationErrors(jdlField, errorsPerValidation) {
