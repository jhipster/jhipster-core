...
function checkApplication(jdlApplication) {
  ...
  checkForAbsentOptions();
  checkForValidValues();
  checkForInvalidDatabaseCombinations();
}

function checkForAbsentOptions() {
  const absentOptions = [];
  ['baseName', 'authenticationType', 'buildTool'].forEach(option => {
    if (!applicationConfig[option]) {
      absentOptions.push(option);
    }
  });
  if (absentOptions.length !== 0) {
    const plural = absentOptions.length > 1;
    throw new Error(
      `The application option${plural ? 's' : ''} ${absentOptions.join(', ')} ${plural ? 'were not' : 'was not'} found.`
    );
  }
  if (applicationConfig.enableTranslation && !applicationConfig.nativeLanguage) {
