...
function getRelationship(relationship, entityName) {
  ...
  relationshipConfiguration.sourceEntity = entityName;
  relationshipConfiguration.injectedFieldInSourceEntity = getInjectedFieldInSourceEntity(relationship);
  relationshipConfiguration.injectedFieldInSourceIsRequired = relationship.relationshipValidateRules;
  relationshipConfiguration.commentForSourceEntity = relationship.javadoc;
  relationshipConfiguration.options = relationship.options || relationshipConfiguration.options;
  ...
  if (relationship.relationshipType === 'many-to-one') {
    if (relationshipConfiguration.injectedFieldInDestinationEntity) {
      ...
      return getBidirectionalOneToManyRelationship(relationship, relationshipConfiguration);
    }
  ...
  if (relationshipConfiguration.type) {
    ...
    return getDefaultRelationship(relationship, relationshipConfiguration);
  }
  return undefined;
}
...
function getDestinationEntitySideAttributes(
  destinationEntityNameFromRelationship,
  entityName,
  destinationEntityRelationships
) {
  for (let i = 0; i < destinationEntityRelationships.length; i++) {
    const destinationEntityFromRelationship = destinationEntityRelationships[i];
    const isEntityTheDestinationEntity = () =>
      upperFirst(destinationEntityFromRelationship.otherEntityName) === entityName &&
      destinationEntityFromRelationship.otherEntityRelationshipName === destinationEntityNameFromRelationship;
    if (isEntityTheDestinationEntity()) {
      // Bidirectional relationship
      let injectedFieldInDestinationEntity = destinationEntityFromRelationship.relationshipName;
      if (
        !!destinationEntityFromRelationship.otherEntityField &&
        destinationEntityFromRelationship.otherEntityField !== 'id'
      ) {
        injectedFieldInDestinationEntity += `(${destinationEntityFromRelationship.otherEntityField})`;
      }
      const injectedFieldInDestinationIsRequired = !!destinationEntityFromRelationship.relationshipValidateRules;
      const commentForDestinationEntity = destinationEntityFromRelationship.javadoc;
      return {
...
function getDefaultRelationship(relationship, relationshipConfiguration) {
...
function getBidirectionalOneToManyRelationship(relationship, relationshipConfiguration) {
