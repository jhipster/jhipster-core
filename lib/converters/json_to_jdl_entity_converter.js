...
function init(entities, jdl) {
  ...
  configuration.jdlObject = jdl || configuration.jdlObject;
  configuration.skippedUserManagement = hasSkipUserManagement();
}
...
function addEntity(entityName) {
  if (entityName === USER && !configuration.skippedUserManagement) {
...
function shouldHandleUserEntity(relationship) {
  return relationship.otherEntityName.toLowerCase() === USER.toLowerCase() && !configuration.skippedUserManagement;
}
