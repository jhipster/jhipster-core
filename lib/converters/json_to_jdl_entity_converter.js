...
function getDestinationEntitySideAttributes(relationship, entityName, destinationEntityRelationships) {
  for (let i = 0; i < destinationEntityRelationships.length; i++) {
    const relationship2 = destinationEntityRelationships[i];
    if (
      ...
      let injectedFieldInDestinationEntity = relationship2.relationshipName;
      if (!!relationship2.otherEntityField && relationship2.otherEntityField !== 'id') {
        injectedFieldInDestinationEntity += `(${relationship2.otherEntityField})`;
      }
      const injectedFieldInDestinationIsRequired = !!relationship2.relationshipValidateRules;
      const commentForDestinationEntity = relationship2.javadoc;
      return {
