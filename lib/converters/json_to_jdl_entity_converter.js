...
function init(params) {
  ...
  jdlObject = params.jdlObject || new JDLObject();
  skipUserManagement = hasSkipUserManagement();
}

function hasSkipUserManagement() {
  return jdlObject.hasOption(SKIP_USER_MANAGEMENT);
}
...
function addEntity(entityName) {
  if (entityName === USER_ENTITY_NAME && !skipUserManagement) {
...
function addRelationshipsToJDL() {
  for (let i = 0, entityNames = Object.keys(entities); i < entityNames.length; i++) {
    dealWithRelationships(entities[entityNames[i]].relationships, entityNames[i]);
  }
}
...
function getRelationship(relationship, entityName) {
  ...
  if (shouldHandleUserEntity(relationship)) {
    ...
    relationshipConfiguration.otherEntity = entities[upperFirst(relationship.otherEntityName)];
...
function shouldHandleUserEntity(relationship) {
  return relationship.otherEntityName.toLowerCase() === USER_ENTITY_NAME.toLowerCase() && !skipUserManagement;
}

function setUpRelationshipConfiguration(relationship, entityName, relationshipConfiguration) {
  relationshipConfiguration.toJdlEntity = jdlObject.getEntity(upperFirst(relationship.otherEntityName));
