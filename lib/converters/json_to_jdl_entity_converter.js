...
function convertEntitiesToJDL(params) {
  ...
  init(params.entities, params.jdlObject || new JDLObject());
  addEntities();
  addRelationshipsToJDL();
  return configuration.jdlObject;
}

function init(entities, jdlObject) {
  configuration.entities = entities;
  configuration.jdlObject = jdlObject || configuration.jdlObject;
  configuration.skipUserManagement = hasSkipUserManagement();
}

function hasSkipUserManagement() {
  return configuration.jdlObject.hasOption(SKIP_USER_MANAGEMENT);
}
...
function addEntity(entityName) {
  if (entityName === USER_ENTITY_NAME && !configuration.skipUserManagement) {
  ...
  configuration.jdlObject.addEntity(convertJSONToJDLEntity(entity, entityName));
...
function addEnumsToJDL(entity) {
  entity.fields.forEach(field => {
    if (field.fieldValues !== undefined) {
      configuration.jdlObject.addEnum(
        new JDLEnum({
...
function dealWithRelationships(relationships, entityName) {
  ...
  relationships.forEach(relationship => {
    ...
    if (jdlRelationship) {
      configuration.jdlObject.addRelationship(jdlRelationship);
    }
  });
}
...
function shouldHandleUserEntity(relationship) {
  return (
    relationship.otherEntityName.toLowerCase() === USER_ENTITY_NAME.toLowerCase() && !configuration.skipUserManagement
  );
}

function setUpRelationshipConfiguration(relationship, entityName, relationshipConfiguration) {
  relationshipConfiguration.toJdlEntity = configuration.jdlObject.getEntity(upperFirst(relationship.otherEntityName));
...
function addUnaryOptionToJDL(unaryOption, entityName) {
  configuration.jdlObject.addOption(
    new JDLUnaryOption({
      name: unaryOption,
