...
function getRelationship(relationship, entityName) {
  const relationshipConfiguration = {
    ...
    otherEntity: null,
    destinationJDLEntity: null,
    options: []
  };

  if (shouldHandleUserEntity(relationship)) {
    relationshipConfiguration.destinationJDLEntity = USER_ENTITY;
  } else {
    relationshipConfiguration.otherEntity = entities.get(upperFirst(relationship.otherEntityName));
...
function setUpRelationshipConfiguration(relationship, entityName, relationshipConfiguration) {
  relationshipConfiguration.destinationJDLEntity = jdlObject.getEntity(upperFirst(relationship.otherEntityName));
  for (let i = 0; i < relationshipConfiguration.otherEntity.relationships.length; i++) {
    ...
    if (
      ...
      relationshipConfiguration.injectedFieldInDestinationEntity = relationship2.relationshipName;
      if (!!relationship2.otherEntityField && relationship2.otherEntityField !== 'id') {
        relationshipConfiguration.injectedFieldInDestinationEntity += `(${relationship2.otherEntityField})`;
      }
      relationshipConfiguration.injectedFieldInDestinationIsRequired = !!relationship2.relationshipValidateRules;
      relationshipConfiguration.commentForDestinationEntity = relationship2.javadoc;
      break;
    }
  }
}
...
function getDefaultRelationship(relationship, entityName, relationshipConfiguration) {
  const jdlRelationship = new JDLRelationship({
    ...
    from: entityName,
    to: relationshipConfiguration.destinationJDLEntity.name,
    type: relationshipConfiguration.type,
    injectedFieldInFrom: getInjectedFieldInSourceEntity(relationship),
...
function getBidirectionalOneToManyRelationship(relationship, entityName, relationshipConfiguration) {
  const jdlRelationship = new JDLRelationship({
    ...
    injectedFieldInTo: getInjectedFieldInSourceEntity(relationship),
