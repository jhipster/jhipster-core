 ...
 * @param {JDLObject} jdlObject - the JDL object containing the options.
...
function convert(jdlObject) {
  if (!jdlObject) {
    throw new Error('A JDL Object must be passed to convert JDL options to JSON.');
  }
  convertedOptionContent = new Map();
  resolveEntityNamesForEachOption(jdlObject);
  setConvertedOptionContents(jdlObject);
  return convertedOptionContent;
}

function resolveEntityNamesForEachOption(jdlObject) {
...
function setConvertedOptionContents(jdlObject) {
  jdlObject.forEachOption(jdlOption => {
