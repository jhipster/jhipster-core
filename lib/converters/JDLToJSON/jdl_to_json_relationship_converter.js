...
function setRelationshipsFromEntity(relatedRelationships, entityName) {
  relatedRelationships.from.forEach(relationshipToConvert => {
    ...
    if (relationshipToConvert.type === ONE_TO_ONE) {
      convertedRelationship.relationshipName =
        camelCase(splitField.relationshipName) || camelCase(relationshipToConvert.to);
      ...
      //     otherEntityRelationshipName: camelCase(relationshipToConvert.to),
      //     relationshipType: _.kebabCase(MANY_TO_ONE),
      //     otherEntityField: lowerFirst(otherSplitField.otherEntityField)
      //   };
      //   relationshipToConvert.type = MANY_TO_ONE;
      //   entities[relationshipToConvert.to].addRelationship(otherSideRelationship);
      // }
...
function setOptionsForRelationship(relationshipToConvert, convertedRelationship) {
  if (Object.keys(relationshipToConvert.options).length === 0) {
    return;
  }
  Object.entries(relationshipToConvert.options).forEach(([key, value]) => {
    if (key === JPA_DERIVED_IDENTIFIER) {
      if (convertedRelationship.ownerSide) {
        convertedRelationship.useJPADerivedIdentifier = value;
      }
    } else {
      if (!convertedRelationship.options) {
        convertedRelationship.options = {};
      }
      convertedRelationship.options[key] = value;
    }
  });
}
