...
function loadJhipsterConfigAndEntities() {
  ...
  if (FileUtils.doesFileExist(path.join('.', '.yo-rc.json'))) {
    apps.push(loadApplicationAndEntities('.'));
  } else {
    listYoRcDirectories().forEach(dir => apps.push(loadApplicationAndEntities(path.join('.', dir))));
  }
 ...
 */
function listYoRcDirectories() {
  return fs
    .readdirSync('.')
    .filter(content => fs.statSync(path.join('.', content)).isDirectory())
    .filter(dir => FileUtils.doesFileExist(path.join('.', dir, '.yo-rc.json')));
}
