...
const alphanumericOrStringLiteral = /^([A-Za-z][A-Za-z0-9]*)|"[^"]+"$/;


const configPropsValidations = {
  BASE_NAME: {
    type: 'NAME_OR_STRING',
    pattern: alphanumericOrStringLiteral,
  ...
  checkExpectedValueType(expected, actual) {
    ...
    case 'NAME_OR_STRING':
      if (!(actual.name === 'qualifiedName'
        ...
        || actual.tokenType === LexerTokens.STRING)) {
        this.errors.push({
          message: `A name or string literal is expected, but found: "${getFirstToken(actual).image}"`,
