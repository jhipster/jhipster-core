...
createToken({ name: 'PATH', pattern: 'path', push_mode: 'freeStyle' });
...
// The base tokens include everything except the "freeStyle" line
// TODO: maybe we need to be more explicit about the exclusion (i.e use _.filter).
// as this code is dependent on the order of statements
const baseTokens = _.values(tokens);

createToken({ name: 'LINE', pattern: /[^\r\n]+/, pop_mode: true });
...
const JDLLexer = new Lexer(lexerDefinition, { debug: true });
