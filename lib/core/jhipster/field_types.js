...
const UNION_COMMON_TYPES = Object.assign(CommonDBTypes, FIELD_LOCATION);
const UNION_CASSANDRA_TYPES = Object.assign(CassandraTypes, FIELD_LOCATION);

function isCommonDBType(type) {
  if (!type) {
    throw new Error('The passed type must not be nil.');
  }
  return _.snakeCase(type).toUpperCase() in UNION_COMMON_TYPES || type instanceof JDLEnum;
}
...
function isCassandraType(type) {
  ...
  return _.snakeCase(type).toUpperCase() in UNION_CASSANDRA_TYPES && !(type instanceof JDLEnum);
}
